...
export class newScene extends Component {
  ...
  setPopupContent = () => {
    ...
    self.geoMarker
      .bindPopup('LatLng: ' + lat + ', ' + lng + '<br/>' + 'Zoom: ' + zoom)
      .openPopup()
      .openPopup();
  };
  updatePopup = () => {
  ...
  handleSubmit(event) {
    ...
    if (document.getElementById('selectLoc').checked) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        function(position) {
          var geo = { latitude: -34.397, longitude: 150.644 };
          fetch('/stations/new', {
            method: 'POST',
            mode: 'cors',
            headers: {
      ...
      navigator.geolocation.getCurrentPosition(
        function(position) {
          ...
          if (
            ...
            document
              .getElementById('mapPicker')
              .appendChild(container.mapContainer.mapContainer);
            ...
            if (self.map == undefined) {
              self.map = L.Wrld.map(
                'mapPicker',
                'efa27b86ca40c4d3babe71bc4d9b9e9f',
                {
          ...
          if (self.geoMarker == undefined) {
            self.geoMarker = L.Wrld.marker(self.map.getCenter()).addTo(
              self.map
            );
      ...
      <div className={styles.container}>
        <div className={styles.appbar}>
          <UIAppBar
            ...
            leftIcon={navigationBackButton(this.openTemplates)}
          />
        </div>
